...
class TopicResponseController extends GetxController {
  PaudRepository paudRepository;
  Rx<ScreenState> viewState = Rx<ScreenState>(InitialScreenState());
  Rx<File> selectedFile = Rx<File>(File(""));
  File fileResponse = File("");
  String errorMessage = "";

  TopicResponseController({required this.paudRepository});
  ...
  void onInit() {
    ...
    ever(viewState, (state) {
      if (state is LoadingScreenState) {
        ...
        Get.back();
        showDialogSuccess(
            message: "Response telah berhasil dikirim",
            onClosed: () {
              Get.back();
            });
      }
  ...
  void setFileResponse(File file) {
    fileResponse = file;
  }

  void postTopicResponse(DIO.FormData request) async {
    viewState.value = LoadingScreenState();

    try {
      ApiResponse response = await paudRepository.postResponseTopic(request);
