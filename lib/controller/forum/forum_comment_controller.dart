...
class ForumCommentController extends GetxController {
  PaudRepository paudRepository;
  Rx<ScreenState> viewState = Rx<ScreenState>(InitialScreenState());
  String errorMessage = "";

  ForumCommentController({required this.paudRepository});
  ...
  void postComment(DIO.FormData request) async {
    viewState.value = LoadingScreenState();

    try {
      ApiResponse result = await paudRepository.postFormComment(request);
