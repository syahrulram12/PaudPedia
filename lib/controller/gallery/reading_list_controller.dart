...
class ReadingListController extends GetxController {
  ...
  Rx<ApiResponse<List<ReadingListResponse>>> reads =
      Rx<ApiResponse<List<ReadingListResponse>>>(ApiResponse());
  String errorMessage = "";
  int screenwidth = 0;
  Rx<double> progressDownload = 0.0.obs;

  ReadingListController({required this.paudRepository});
  ...
  void getReadingList() async {
    ...
    try {
      reads.value = await paudRepository.getReadingList();
